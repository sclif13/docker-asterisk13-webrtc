...
WORKDIR /usr/src
RUN wget http://downloads.asterisk.org/pub/telephony/asterisk/asterisk-13.5.0.tar.gz && tar -zxvf asterisk-13.5.0.tar.gz

WORKDIR /usr/src/asterisk-13.5.0/contrib/scripts
RUN ./install_prereq install && ./install_prereq install-unpackaged && ./get_mp3_source.sh

WORKDIR /usr/src/asterisk-13.5.0
